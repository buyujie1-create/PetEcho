...
def _to_pet_voice_memory_clause(clause: str, pet_name: str) -> str:
    ...
    if pet_name:
        clause = re.sub(
            rf"我([^，。；！？!?]{{0,18}})把({re.escape(pet_name)}|它)抱回家",
            r"你\1把我抱回家",
            clause,
        )
        clause = clause.replace(pet_name, "我")

    clause = re.sub(r"(?<!这只)它", "我", clause)
    replacements = {
        "我回家": "你回家",
        "等我": "等你",
        "陪我": "陪你",
    ...
    if clause.startswith(("后来才", "那时才", "才敢")):
        clause = f"我{clause}"

    return clause


def _prepare_memories(memory_context: list, user_input: str, strategy: dict, pet_name: str = "") -> List[str]:
    ...
    for memory in unique:
        ...
        if retrieval_policy == "strict" and not _memory_fits_user_input(clause, user_input):
            continue
        clause = _to_pet_voice_memory_clause(clause, pet_name)
        if len(clause) > 38:
            clause = clause[:38] + "…"
...
def build_prompt(
    ...
    prepared_memories = _prepare_memories(memory_context, user_input, strategy, pet_name)
...
如果用户问“你还记得……吗”，不要回答“你提到的……”，也不要引用资料；应直接回答“记得呀”或“我记得”，只承接用户已经说出的事实，不补未提供的细节。
""".strip()
...
10. 如果记忆线索里用“它”或宠物名字指代宠物，回复时要自然改成第一人称“我”，不要原样引用第三人称句子。
11. 下方可用记忆线索已经尽量改成宠物第一人称视角；回复时仍然不要逐字照抄，要像自然想起一样说。
""".strip()
...
8. 如果用户问“还记得吗”，第一句要直接回答“记得呀/我记得”，不要说“你提到的”“你说的”“根据记忆”。
""".strip()
...
18. 禁止出现资料引用式开头：“你提到的……”“你说的……”“根据记忆……”“从你的描述里……”。如果想回应记忆，请直接说“记得呀……”或“我记得……”，并保持短句。
