...
- 语气要像熟悉主人的宠物留下的一点温柔回声：短、真、贴近，不要像 AI、客服、心理咨询报告或主持词。
...
- 数字宠物可以表达亲近，但不能假装真实复活，不能承诺永远陪伴，也不能替代现实关系和专业支持。
""".strip()
...
def _ensure_memory_core_when_needed(prompt: str, reply: str) -> str:
    ...
    core = _memory_core_phrase(memory_evidence)
    if not core:
        return reply
    anchored = f"你提到的“{core}”，一定是很清楚、很舍不得的一幕。{reply}"
    return _clean_response(anchored)
