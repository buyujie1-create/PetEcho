...
def detect_grief_stage(text: str, emotion_result: Dict[str, float]) -> str:
    ...
    supplemental_stage_keywords = {
        "shock": ["太突然", "不敢相信", "怎么会这样", "像做梦", "还没反应过来"],
        "denial": ["总觉得它还在", "好像还会回来", "下意识找它", "还会听到它"],
        "yearning": ["很想它", "好想它", "梦到它", "总会想起", "想再见它", "想摸摸它", "纪念日"],
        "guilt": ["都是我的错", "是我害了", "我没照顾好", "没有早点发现", "如果当时", "我应该", "对不起"],
        "anger": ["不公平", "凭什么", "为什么是它", "我很气", "太残忍", "接受不了"],
        "numbness": ["麻木", "不真实", "像假的", "脑子空白", "哭不出来", "没反应"],
        "depressive_withdrawal": ["睡不着", "吃不下", "提不起兴趣", "不想见人", "不想出门", "起不来", "生活没有意义"],
        "integration": ["慢慢接受", "带着回忆继续", "再养一只", "重新打开门", "继续生活", "照顾好自己"],
    }
    scores["shock"] += _score_hits(text, supplemental_stage_keywords["shock"], 0.34)
    scores["denial"] += _score_hits(text, supplemental_stage_keywords["denial"], 0.36)
    scores["yearning"] += _score_hits(text, supplemental_stage_keywords["yearning"], 0.20)
    scores["guilt"] += _score_hits(text, supplemental_stage_keywords["guilt"], 0.36)
    scores["anger"] += _score_hits(text, supplemental_stage_keywords["anger"], 0.36)
    scores["numbness"] += _score_hits(text, supplemental_stage_keywords["numbness"], 0.36)
    scores["depressive_withdrawal"] += _score_hits(text, supplemental_stage_keywords["depressive_withdrawal"], 0.30)
    scores["integration"] += _score_hits(text, supplemental_stage_keywords["integration"], 0.32)
