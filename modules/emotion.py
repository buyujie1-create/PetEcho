import re
from typing import Dict


def _normalize_text(text: str) -> str:
    text = text or ""
    text = text.strip()
    text = re.sub(r"\s+", "", text)
    return text


def _hit_score(text: str, weighted_terms: dict[str, float]) -> float:
    score = 0.0
    for term, weight in weighted_terms.items():
        if term in text:
            score += weight
    return score


def _regex_score(text: str, weighted_patterns: list[tuple[str, float]]) -> float:
    score = 0.0
    for pattern, weight in weighted_patterns:
        if re.search(pattern, text):
            score += weight
    return score


def _clamp(x: float) -> float:
    return round(max(0.0, min(1.0, x)), 2)


def detect_emotion(text: str) -> Dict[str, float]:
    text = _normalize_text(text)

    if not text:
        return {
            "sadness": 0.0,
            "loneliness": 0.0,
            "yearning": 0.0,
            "guilt": 0.0,
            "anger": 0.0,
            "numbness": 0.0,
        }

    sadness_terms = {
        "难过": 0.30,
        "伤心": 0.34,
        "舍不得": 0.40,
        "好难受": 0.42,
        "痛苦": 0.48,
        "崩溃": 0.58,
        "想哭": 0.32,
        "哭": 0.18,
        "失落": 0.28,
        "低落": 0.28,
        "心里空": 0.34,
        "心里很空": 0.42,
        "空空的": 0.28,
        "不在了": 0.26,
        "离开了": 0.24,
        "再也": 0.18,
        "受不了": 0.24,
        "压得喘不过气": 0.46,
        "提不起劲": 0.28,
        "没有力气": 0.28,
        "很累": 0.18,
    }

    loneliness_terms = {
        "孤独": 0.50,
        "一个人": 0.30,
        "空落落": 0.52,
        "空荡荡": 0.42,
        "没人陪": 0.58,
        "冷清": 0.30,
        "安静得可怕": 0.42,
        "没有陪我": 0.36,
        "没人在": 0.24,
        "回家很空": 0.34,
        "好安静": 0.24,
        "家里太安静": 0.40,
    }

    yearning_terms = {
        "想你": 0.62,
        "很想你": 0.78,
        "好想你": 0.82,
        "我想你": 0.72,
        "想它": 0.70,
        "很想它": 0.80,
        "好想它": 0.82,
        "想念": 0.58,
        "怀念": 0.50,
        "惦记": 0.34,
        "想起": 0.20,
        "记得": 0.14,
        "以前": 0.16,
        "那时候": 0.18,
        "回忆": 0.18,
        "又想起": 0.26,
        "总会想起": 0.30,
    }

    guilt_terms = {
        "愧疚": 0.62,
        "内疚": 0.58,
        "自责": 0.56,
        "后悔": 0.42,
        "怪我": 0.46,
        "都是我的错": 0.78,
        "是我的错": 0.62,
        "我没有照顾好": 0.66,
        "早知道": 0.34,
        "如果我": 0.20,
        "我应该": 0.24,
        "对不起": 0.30,
    }

    anger_terms = {
        "不公平": 0.54,
        "为什么是它": 0.50,
        "为什么偏偏": 0.46,
        "生气": 0.40,
        "愤怒": 0.52,
        "怨": 0.28,
        "恨": 0.32,
        "接受不了": 0.30,
        "太残忍": 0.38,
    }

    numbness_terms = {
        "麻木": 0.62,
        "没感觉": 0.44,
        "像假的": 0.40,
        "不真实": 0.42,
        "空白": 0.32,
        "哭不出来": 0.36,
        "没有反应": 0.32,
    }

    sadness = _hit_score(text, sadness_terms)
    loneliness = _hit_score(text, loneliness_terms)
    yearning = _hit_score(text, yearning_terms)
    guilt = _hit_score(text, guilt_terms)
    anger = _hit_score(text, anger_terms)
    numbness = _hit_score(text, numbness_terms)

    supplemental_terms = {
        "sadness": {
            "失去": 0.18,
            "离开了": 0.22,
            "走了": 0.18,
            "没了": 0.18,
            "空了一块": 0.36,
            "心里很空": 0.38,
            "很沉": 0.24,
            "喘不过气": 0.38,
            "提不起兴趣": 0.28,
            "不想说话": 0.22,
        },
        "loneliness": {
            "家里好空": 0.42,
            "没人陪": 0.42,
            "只剩我": 0.38,
            "一个人回家": 0.34,
            "太安静": 0.28,
        },
        "yearning": {
            "很想它": 0.78,
            "好想它": 0.82,
            "想再见": 0.62,
            "想摸摸": 0.52,
            "想抱抱": 0.52,
            "梦到它": 0.48,
            "总会想起": 0.42,
            "纪念日": 0.28,
        },
        "guilt": {
            "都是我的错": 0.80,
            "是我害了": 0.78,
            "怪我": 0.55,
            "我没照顾好": 0.70,
            "没有早点发现": 0.62,
            "如果当时": 0.32,
            "我应该": 0.28,
            "对不起": 0.34,
            "责备我": 0.38,
            "怪我吗": 0.40,
            "会不会怪我": 0.46,
            "背叛": 0.34,
        },
        "anger": {
            "不公平": 0.58,
            "凭什么": 0.48,
            "为什么是它": 0.52,
            "我很气": 0.48,
            "接受不了": 0.34,
            "太残忍": 0.40,
        },
        "numbness": {
            "麻木": 0.64,
            "不真实": 0.48,
            "像假的": 0.44,
            "像做梦": 0.40,
            "脑子空白": 0.40,
            "哭不出来": 0.38,
            "没反应": 0.34,
        },
    }
    sadness += _hit_score(text, supplemental_terms["sadness"])
    loneliness += _hit_score(text, supplemental_terms["loneliness"])
    yearning += _hit_score(text, supplemental_terms["yearning"])
    guilt += _hit_score(text, supplemental_terms["guilt"])
    anger += _hit_score(text, supplemental_terms["anger"])
    numbness += _hit_score(text, supplemental_terms["numbness"])

    sadness += _regex_score(text, [
        (r"(再也|已经).{0,4}(见不到|摸不到|听不到|不能)", 0.28),
        (r"(睡不着|吃不下|没胃口)", 0.16),
        (r"(怎么会|为什么会这样)", 0.12),
    ])

    loneliness += _regex_score(text, [
        (r"(没有|没).{0,4}(陪|在身边)", 0.26),
        (r"(回家|晚上|一个人).{0,6}(很空|好空|空落落|冷清)", 0.24),
        (r"(家里|房间|床边|门口|回家).{0,8}(空|安静|冷清)", 0.34),
    ])

    yearning += _regex_score(text, [
        (r"(想|想念|怀念).{0,2}(你|它)", 0.28),
        (r"(还记得|记不记得|以前|那时候)", 0.10),
        (r"(如果你还在|要是你还在)", 0.16),
    ])

    guilt += _regex_score(text, [
        (r"(如果|要是).{0,8}(我|当时).{0,8}(早点|多陪|发现|带它去)", 0.24),
        (r"(我).{0,6}(害了|耽误了|没救到|没保护好)", 0.42),
        (r"(是不是|是不是因为).{0,8}(我|我的错)", 0.30),
    ])

    anger += _regex_score(text, [
        (r"(为什么|凭什么).{0,8}(它|这么快|这样)", 0.22),
        (r"(我).{0,5}(很气|好气|生气|愤怒)", 0.24),
    ])

    numbness += _regex_score(text, [
        (r"(像|好像).{0,4}(做梦|假的|不是真的)", 0.26),
        (r"(脑子|心里).{0,4}(空白|麻木)", 0.24),
    ])

    # 强度词修正
    intensity_terms = ["很", "特别", "真的", "一直", "总是", "太", "非常", "一下子"]
    if any(term in text for term in intensity_terms):
        if sadness > 0:
            sadness += 0.05
        if loneliness > 0:
            loneliness += 0.05
        if yearning > 0:
            yearning += 0.06
        if guilt > 0:
            guilt += 0.05
        if anger > 0:
            anger += 0.05
        if numbness > 0:
            numbness += 0.04

    # 联动关系：思念往往伴随一定悲伤；强孤单也会带一点悲伤
    if yearning >= 0.55 and sadness < 0.24:
        sadness += 0.14
    if loneliness >= 0.50 and sadness < 0.24:
        sadness += 0.10

    # 如果用户主要在叙事回忆，不一定非常痛，但会有思念
    if yearning == 0 and re.search(r"(记得|以前|那时候|总会|每次)", text):
        yearning += 0.18

    return {
        "sadness": _clamp(sadness),
        "loneliness": _clamp(loneliness),
        "yearning": _clamp(yearning),
        "guilt": _clamp(guilt),
        "anger": _clamp(anger),
        "numbness": _clamp(numbness),
    }
