...
with st.container(border=True):
    ...
    status_cols4 = st.columns(4, gap="medium")
    with status_cols4[0]:
        render_metric_card("聊天转录", f"{csv_row_count(CHAT_TRANSCRIPT_PATH)} 条", CHAT_TRANSCRIPT_PATH)
...
with st.container(border=True):
    ...
    download_cols = st.columns(4, gap="medium")
    ...
    with download_cols[2]:
        st.download_button(
            "下载练习记录 CSV",
            data=csv_file_bytes(REFLECTION_PATH, REFLECTION_FIELDNAMES),
    ...
    with download_cols[3]:
        st.download_button(
            "下载聊天转录 CSV",
            data=csv_file_bytes(CHAT_TRANSCRIPT_PATH, CHAT_TRANSCRIPT_FIELDNAMES),
            file_name="petecho_chat_transcripts.csv",
            mime="text/csv",
            use_container_width=True,
        )
