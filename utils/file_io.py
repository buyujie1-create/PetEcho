...
GENERATED_AVATAR_PATH = os.path.join(GENERATED_DIR, "pet_virtual_avatar.png")
...
# 生成式纪念形象
# ---------------------------
def save_generated_pet_avatar(image_bytes: bytes) -> str:
    ensure_data_dir()
    with open(GENERATED_AVATAR_PATH, "wb") as f:
        f.write(image_bytes)
    return GENERATED_AVATAR_PATH


def load_generated_pet_avatar_path() -> str | None:
    if os.path.exists(GENERATED_AVATAR_PATH):
        return GENERATED_AVATAR_PATH
    return None


def clear_generated_pet_avatar():
    if os.path.exists(GENERATED_AVATAR_PATH):
        try:
            os.remove(GENERATED_AVATAR_PATH)
        except Exception:
            pass
