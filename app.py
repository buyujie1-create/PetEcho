...
.message-row.left { justify-content: flex-start; align-items: flex-start; }
.message-row.right { justify-content: flex-end; align-items: flex-start; }
.message-col-left, .message-col-right {
    max-width: 76%;
    display: flex;
    flex-direction: column;
}
.message-col-left { align-items: flex-start; }
.message-col-right { align-items: flex-end; }
.avatar {
    width: 46px;
    height: 46px;
    min-width: 46px;
...
.meta-left { margin-left: 4px; }
.meta-right { margin-right: 4px; }
.pet-name { color: #c65c7b; font-weight: 700; }
.user-name { color: #4d79d8; font-weight: 700; }
.msg-time { color: #98a2b3; font-size: 0.78rem; }
.pet-bubble {
    background: #fff7f8;
    color: #2b2b2b;
...
def reset_runtime_state() -> None:
    keys_to_clear = [
        "chat_history","last_reply","last_meta","pet_name_input",
        "pet_personality_input","pet_appearance_input","pet_memories_input",
        "chat_input","pending_chat_input",
    ]
...
def get_default_cat_avatar_data_uri() -> str:
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
      ...
      <stop offset="0%" stop-color="#ffd6a5"/><stop offset="100%" stop-color="#ffc8dd"/></linearGradient></defs>
      <rect width="96" height="96" rx="48" fill="url(#bg)"/>
      <polygon points="24,34 36,16 42,36" fill="#f5a3b7"/><polygon points="72,34 60,16 54,36" fill="#f5a3b7"/>
      <circle cx="48" cy="52" r="28" fill="#fff6f0"/>
      <circle cx="37" cy="50" r="5" fill="#333"/><circle cx="59" cy="50" r="5" fill="#333"/>
      <circle cx="35.5" cy="48.5" r="1.6" fill="#fff"/><circle cx="57.5" cy="48.5" r="1.6" fill="#fff"/>
...
def get_user_avatar_data_uri() -> str:
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
      <defs><linearGradient id="bg" x1="0" x2="1" y1="0" y2="1">
      <stop offset="0%" stop-color="#bfd7ff"/><stop offset="100%" stop-color="#dbeafe"/></linearGradient></defs>
      <rect width="96" height="96" rx="48" fill="url(#bg)"/>
      <circle cx="48" cy="38" r="16" fill="#ffffff"/><path d="M24 78 Q48 56 72 78" fill="#ffffff"/>
      <circle cx="42" cy="37" r="2.4" fill="#4b5563"/><circle cx="54" cy="37" r="2.4" fill="#4b5563"/>
...
def render_message(role: str, text: str, name: str, timestamp: str) -> None:
    ...
    if role == "user":
        ...
        st.markdown(f'''
            <div class="chat-container"><div class="message-row right">
            <div class="message-col-right"><div class="meta-right">
            <span class="msg-time">{safe_time}</span><span class="user-name">{safe_name}</span></div>
            <div class="user-bubble">{safe_text}</div></div>
            <div class="avatar"><img src="{avatar_uri}" alt="user avatar"></div></div></div>
        ''', unsafe_allow_html=True)
    else:
        avatar_uri = get_pet_avatar_data_uri()
        st.markdown(f'''
            <div class="chat-container"><div class="message-row left">
            <div class="avatar"><img src="{avatar_uri}" alt="pet avatar"></div>
            <div class="message-col-left"><div class="meta-left">
            <span class="pet-name">{safe_name}</span><span class="msg-time">{safe_time}</span></div>
            <div class="pet-bubble">{safe_text}</div></div></div></div>
        ''', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.button("载入演示数据", width="stretch"):
        ...
        st.success("已载入演示数据。")
        st.rerun()
    if st.button("重置所有宠物数据", width="stretch"):
        reset_pet_data()
        reset_runtime_state()
        st.success("已重置所有数据。")
        st.rerun()
    st.markdown("---")
    st.caption("在线演示版已关闭图像描述模型，以降低内存占用并提升多人访问稳定性。")
...
with left_col:
    with st.container(border=True):
        ...
        ''', unsafe_allow_html=True)
        st.markdown('<div class="feature-note">在线演示版已关闭“根据照片自动生成外观描述”功能，以降低云端内存占用。你仍可上传宠物照片，并手动填写外观描述；Embedding 记忆检索与对话功能保持启用。</div>', unsafe_allow_html=True)

        pet_name = st.text_input("宠物名字", key="pet_name_input")
        pet_personality = st.text_area("宠物性格", placeholder="例如：很黏人，喜欢散步，见到我会很兴奋", key="pet_personality_input")
        pet_appearance = st.text_area("宠物外观描述（请手动填写，在线演示版暂不启用照片自动生成）", placeholder="例如：毛很蓬松，白色身体，脸和耳朵偏灰，眼睛圆圆的，像一团棉花", key="pet_appearance_input")
        pet_memories = st.text_area("宠物回忆", placeholder="例如：它最喜欢傍晚陪我去公园，雨天会缩在我的脚边", key="pet_memories_input", height=150)
        uploaded_image = st.file_uploader("上传宠物照片", type=["jpg", "jpeg", "png"], key="pet_image_uploader")

        btn_col1, btn_col2 = st.columns(2)
        with btn_col1:
            st.button("根据当前照片生成外观描述（在线版已关闭）", width="stretch", disabled=True, help="为降低内存占用，在线演示版已关闭 BLIP 图像描述功能。")
        with btn_col2:
            if st.button("保存宠物档案", width="stretch"):
...
with right_col:
    with st.container(border=True):
        ...
        if uploaded_image is not None:
            st.image(uploaded_image, caption="当前上传图片", width="stretch")
        elif preview_image_path and os.path.exists(preview_image_path):
            st.image(preview_image_path, caption="宠物头像", width="stretch")
        else:
            st.info("还没有上传宠物照片")
...
with st.container(border=True):
    ...
    if st.session_state["chat_history"]:
        ...
        st.markdown('<div class="empty-chat-box">还没有开始对话。你可以先输入一句话，例如：“我很想你”。</div>', unsafe_allow_html=True)

    user_input = st.text_area("你想对它说什么？", height=110, key="chat_input")
    col_send, col_clear = st.columns([1, 1])
    with col_send:
        send_clicked = st.button("发送", width="stretch")
    with col_clear:
        clear_clicked = st.button("清空聊天历史", width="stretch")
        ...
        if not profile:
            ...
            with st.spinner("数字宠物正在想一想该怎么回应你..."):
                ...
                try:
                    ...
                    st.session_state["chat_history"].append({"role": "user", "content": user_input, "timestamp": current_time})
                    st.session_state["chat_history"].append({"role": "assistant", "content": reply, "timestamp": current_time})
...
if st.session_state.get("last_meta"):
    ...
    with st.container(border=True):
        ...
        with c1:
            st.markdown(f'<div class="metric-card"><div class="metric-title">当前阶段</div><div class="metric-value">{meta["grief_stage"]}</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="metric-card"><div class="metric-title">风险等级</div><div class="metric-value">{meta["risk"]["level"]}</div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="metric-card"><div class="metric-title">支持策略</div><div class="metric-value">{meta["strategy"]["name"]}</div></div>', unsafe_allow_html=True)
        with c4:
            ...
            st.markdown(f'<div class="metric-card"><div class="metric-title">回应模式</div><div class="metric-value">{display_value}</div></div>', unsafe_allow_html=True)
